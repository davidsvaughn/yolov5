...
DATA_DIR        = '/home/david/code/phawk/data/fpl/component/models/july12/' ## local save directory
MANIFEST_FILE   = DATA_DIR + 'manifest.txt'
CATEGORIES_FILE = DATA_DIR + 'categories.json'
CLASSES_FILE    = '/home/david/code/phawk/data/fpl/component/coco.names'

LABEL_DIRS = ['/home/david/code/phawk/data/fpl/component/labels']
              # '/home/david/code/phawk/data/fpl/component/new_labels']

S3_IMG_BUCKETS   = ['s3://ai-labeling/FPL/original-labels/',
                    's3://ai-labeling/FPL/components/May18/images/']
...
IMGS = np.array(read_lines('/home/david/code/phawk/data/fpl/component/images_may18.txt'))
def get_s3_bucket_2(file):
    img = file.replace(txt, jpg)
    if img in IMGS:
        s3bucket = S3_IMG_BUCKETS[1]
    else:
        s3bucket = S3_IMG_BUCKETS[0]
    return s3bucket
...
def build_json_string(label_file, name='train', blacklist=None):
    ...
    s3bucket = get_s3_bucket_2(x)
    s3url = s3bucket + x.replace('.txt','.jpg')
...
names = np.array([path_leaf(f).replace('.txt','') for f in lab_files])
testset = np.array(read_lines(DATA_DIR + 'oldtest.txt'))
idx = np.in1d(names, testset)

test_files = lab_files[idx]
val_files = lab_files[idx]
train_files = lab_files[~idx]

X_train = np.array(train_files)[:,None]
y_train = label_matrix(train_files)

X_test = np.array(test_files)[:,None]
y_test = label_matrix(test_files)

X_val = np.array(val_files)[:,None]
y_val = label_matrix(val_files)
